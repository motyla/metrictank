	...
	for isz > 0 {
		...
		case "Metrics":
			...
			for bzg := range z.Metrics {
				z.Metrics[bzg], err = dc.ReadString()
				if err != nil {
					return
				}
			}
...
func (z *IndexFindByTagResp) EncodeMsg(en *msgp.Writer) (err error) {
	...
	for bzg := range z.Metrics {
		err = en.WriteString(z.Metrics[bzg])
		if err != nil {
			return
		}
	}
...
func (z *IndexFindByTagResp) MarshalMsg(b []byte) (o []byte, err error) {
	...
	for bzg := range z.Metrics {
		o = msgp.AppendString(o, z.Metrics[bzg])
	}
	return
}
...
func (z *IndexFindByTagResp) UnmarshalMsg(bts []byte) (o []byte, err error) {
	...
	for isz > 0 {
		...
		case "Metrics":
			...
			for bzg := range z.Metrics {
				z.Metrics[bzg], bts, err = msgp.ReadStringBytes(bts)
				if err != nil {
					return
				}
			}
...
func (z *IndexFindByTagResp) Msgsize() (s int) {
	...
	for bzg := range z.Metrics {
		s += msgp.StringPrefixSize + len(z.Metrics[bzg])
	}
	return
}
